...
If we have context $\Gamma$ and a program $c$ such that $\Gamma \vdash c$, then we define $\alpha^0$ with respects to $\Gamma$.\todo{what does it mean?}
...
\end{itemize}
\todo{not sure about this notation. Could we just write an equality?}
...
\todo{A bit more explanation. Why is $\alpha_{in}^1$ the way it is?}
